...
class ApiController extends Controller
{
    ...
    public function get_fruits(Request $request)
    {
        ...
        $user = auth()->user();
        $agent = $user->agent->id;
        $fruits = AgentStock::with('fruit:id,name,image,sales_price')->where('agent_id',$agent)->where('status',1)->where('stock_pack','>',0)->get()->pluck('fruit')->sortByDesc('name')->values()->all();
